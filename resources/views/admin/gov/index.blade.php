...
<div class="note note-info">
    <h3> @lang('site.view') ( @lang('site.govs') ) </h3>
</div>
            ...
            <div class="portlet-body">
                <div class="table-toolbar">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="btn-group">
                                <a id="sample_editable_1_new" href="{{ route('admin.get.gov.add') }}" class="btn sbold green"> @lang('site.add')
                ...
                @if($govs->count())
               <form action="{{ route('admin.post.gov.deleteMulti') }}" method="post" id="Form2"> @csrf </form>
               <form action="{{ route('admin.post.gov.sort') }}" method="post" id="sortForm">@csrf</form>
                <button  class="btn btn-info btn-sm pull-right sort" type="submit" form="sortForm" >         @lang('site.sort') 
                        ...
                        <tbody class="connected-sortable droppable-area1">
                            ...
                            <tr class="odd gradeX draggable-item">
                                <input type="hidden" name="sort[]" multiple value="{{ $gov->id }}" form="sortForm">
                                <td class="text-center">
                                    <label class="mt-checkbox mt-checkbox-single mt-checkbox-outline">
                                        <input type="checkbox" class="checkboxes" name="deleteMulti[]" form="Form2" multiple value="{{ $gov->id}}" />
                                        <span></span>
                                    </label>
                                </td>
                                <td class="text-center"> 
                                ...
                                <td class="text-center">
                                    @if($gov->desc !== null)
                                        {!! Str::words($gov->desc, 10) !!}
                                    @else
                                        No description available
                                    @endif
                                </td>
                                <td class="text-center">
                                    {!!show_potions('gov',$gov->id)!!}       
                                </td>
