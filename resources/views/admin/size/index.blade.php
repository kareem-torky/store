...
<div class="note note-info">
    <h3> @lang('site.view') ( @lang('site.size') ) </h3>
</div>
            ...
            <div class="portlet-body">
                <div class="table-toolbar">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="btn-group">
                                <a id="sample_editable_1_new" href="{{ route('admin.get.size.add') }}" class="btn sbold green"> @lang('site.add')
                ...
                @if($sizes->count())
               <form action="{{ route('admin.post.size.deleteMulti') }}" method="post" id="Form2"> @csrf </form>
               <form action="{{ route('admin.post.size.sort') }}" method="post" id="sortForm">@csrf</form>
                <button  class="btn btn-info btn-sm pull-right sort" type="submit" form="sortForm" >         @lang('site.sort') 
                        ...
                        <tbody class="connected-sortable droppable-area1">
                            ...
                            <tr class="odd gradeX draggable-item">
                                <input type="hidden" name="sort[]" multiple value="{{ $size->id }}" form="sortForm">
                                <td class="text-center">
                                    <label class="mt-checkbox mt-checkbox-single mt-checkbox-outline">
                                        <input type="checkbox" class="checkboxes" name="deleteMulti[]" form="Form2" multiple value="{{ $size->id}}" />
                                        <span></span>
                                    </label>
                                </td>
                                <td class="text-center"> 
                                ...
                                <td class="text-center">
                                    {!! Str::words($size->desc, 10) !!}
                                </td>
                                <td class="text-center">
                                    {!!show_potions('size',$size->id)!!}       
                                </td>
