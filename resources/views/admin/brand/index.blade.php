...
<div class="note note-info">
    <h3> @lang('site.view') ( @lang('site.brand') ) </h3>
</div>
            ...
            <div class="portlet-body">
                <div class="table-toolbar">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="btn-group">
                                <a id="sample_editable_1_new" href="{{ route('admin.get.brand.add') }}" class="btn sbold green"> @lang('site.add')
                ...
                @if($brands->count())
               <form action="{{ route('admin.post.brand.deleteMulti') }}" method="post" id="Form2"> @csrf </form>
               <form action="{{ route('admin.post.brand.sort') }}" method="post" id="sortForm">@csrf</form>
                <button  class="btn btn-info btn-sm pull-right sort" type="submit" form="sortForm" >         @lang('site.sort') 
                        ...
                        <tbody class="connected-sortable droppable-area1">
                            ...
                            <tr class="odd gradeX draggable-item">
                                <input type="hidden" name="sort[]" multiple value="{{ $bra->id }}" form="sortForm">
                                <td class="text-center">
                                    <label class="mt-checkbox mt-checkbox-single mt-checkbox-outline">
                                        <input type="checkbox" class="checkboxes" name="deleteMulti[]" form="Form2" multiple value="{{ $bra->id}}" />
                                        <span></span>
                                    </label>
                                </td>
                                <td class="text-center"> {{ $bra->name }} </td>

                                <td class="text-center">
                                    <img src="{{ getImage(BRAND_PATH.'small/'.$bra->img) }}" class="privew-image img-thumbnail">
                                </td>
                                <td class="text-center">
                                    {!!show_potions('brand',$bra->id,$bra->status)!!}       
                                </td>
                                <td class="text-center">  
                                  <a href="{{ route('admin.get.brand.seo',$bra->id) }}" class="btn btn-sm btn-primary"> @lang('site.seo') </a> 
                                </td>
                            </tr>
                            @endforeach
